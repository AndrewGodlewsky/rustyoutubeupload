...
pub async fn preform_upload(webdriver: &mut WebDriver, video: &Video) -> WebDriverResult<()> {
    webdriver.goto("https://www.youtube.com").await?;
    crate::cookies::add_cookie(webdriver).await?;
...
pub async fn start(videos: Vec<Video>) -> WebDriverResult<()> {
